...
(function () {
  ...
  root.innerHTML = '<style>\n' +
   ...
   the same values rather than inventing a second brand. */
    ':root{color-scheme:dark;--neutral-rgb:255,255,255;--canvas-rgb:13,11,15;--popup:#1a1520;--bg:#0d0b0f;--glass-fill:rgba(var(--neutral-rgb),.05);--glass-edge:rgba(var(--neutral-rgb),.10);--glass-edge-top:rgba(var(--neutral-rgb),.16);--glass-blur:saturate(150%) blur(18px);--bg-inset:rgba(0,0,0,.30);--surface:var(--glass-fill);--surface-raised:rgba(var(--neutral-rgb),.075);--line:rgba(var(--neutral-rgb),.10);--muted:rgba(var(--neutral-rgb),.68);--dim:rgba(var(--neutral-rgb),.54);--text:#e2e8f0;--accent:#c93545;--accent-soft:#f07a86;--accent-dim:rgba(201,53,69,.15);--secondary:#7fb0f5;--secondary-core:#3b82f6;--amber:#fcd34d;--bad:#fca5a5;--row-h:32px;--radius-sm:8px;--radius-md:10px;--radius-lg:14px;--radius-pill:9999px}' +
    ...
    ':root[data-theme=light]{color-scheme:light;--neutral-rgb:28,25,29;--canvas-rgb:250,249,247;--bg:#faf9f7;--popup:#fff;--glass-fill:rgba(255,255,255,.72);--glass-edge:rgba(28,25,29,.13);--glass-edge-top:rgba(255,255,255,.9);--bg-inset:rgba(28,25,29,.05);--text:#211f24;--muted:#56515b;--dim:#6b6470;--accent-soft:#b5293d;--secondary:#245c9e;--secondary-core:#286bc0;--amber:#795300;--bad:#a52336}' +
   ...
   on the single number this dashboard is about. */
    '#token-observatory .to-header{position:sticky;top:0;z-index:5;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:9px 14px;border-bottom:1px solid var(--line);background:linear-gradient(180deg,rgba(var(--canvas-rgb),.88),rgba(var(--canvas-rgb),.66));backdrop-filter:var(--glass-blur);-webkit-backdrop-filter:var(--glass-blur)}#token-observatory .to-brand{display:flex;gap:9px;align-items:center;min-width:0}#token-observatory .to-mark{width:20px;height:20px;flex:none;color:var(--accent-soft)}#token-observatory h1{position:absolute;width:1px;height:1px;margin:-1px;padding:0;overflow:hidden;clip-path:inset(50%);white-space:nowrap}#token-observatory .to-eyebrow{color:var(--muted);font-size:12px;letter-spacing:0}#token-observatory .to-brand .to-eyebrow{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}' +
...
/* The one remaining piece of foreign chrome was the native select: four OS chevrons
   ...
   palette. The arrow is an inline data: image, which the frame's img-src allows. */
    ...
    ':root[data-theme=light] #token-observatory select{appearance:none;-webkit-appearance:none;padding-right:26px;background-image:url("data:image/svg+xml,%3Csvg%20xmlns%3D%27http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%27%20viewBox%3D%270%200%2010%206%27%3E%3Cpath%20d%3D%27M1%201l4%204%204-4%27%20fill%3D%27none%27%20stroke%3D%27%2356515b%27%20stroke-width%3D%271.4%27%20stroke-linecap%3D%27round%27%2F%3E%3C%2Fsvg%3E");background-repeat:no-repeat;background-position:right 10px center;background-size:10px 6px}' +
  ...
  function trendChart(trend) {
    ...
    [0, .5, 1].forEach(function (fraction) { var yy = top + plot * (1 - fraction); svg += '<line x1="' + left + '" x2="' + (width - right) + '" y1="' + yy + '" y2="' + yy + '" stroke="rgba(var(--neutral-rgb),.10)" stroke-dasharray="3 5"/><text x="0" y="' + (yy + 3) + '">' + esc(number(max * fraction, true)) + '</text>'; });
    keys.forEach(function (key, index) {
      var color = index ? 'var(--secondary)' : 'var(--accent-soft)', segments = [], current = [];
  ...
  // Palette changes update CSS and SVG in place, preserving filters and expanded rows.
  if (window.OuroborosWidget && typeof window.OuroborosWidget.onTheme === 'function') {
